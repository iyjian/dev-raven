...
export class GitLabHookService implements HookParse {
  ...
  parse(data: any): string {
    ...
    if (this[`_${object_kind}Parser`] instanceof Function) {
      ...
      throw new HttpException(`${object_kind} is not supported type`, HttpStatus.NOT_FOUND)
    }
  }

  private async _pushParser(commitData) {
    if (commitData.commits.length === 0) {
      const repositoryUrl = await this.urlService.shortenUrl(commitData.repository.homepage).toPromise()
    ...
    for (const c of commitData.commits) {
      const singleCommitUrl = await this.urlService.shortenUrl(c.url).toPromise()
      const singleCommit = {
        message: c.message,
        code: c.id.slice(0, 8),
        url: singleCommitUrl
  ...
  private async _noteParser(commitData) {
    ...
    if (filteredData.description.match(/(\!\[\S+\]\(\S+\))/)) {
      filteredData.description = await this._shortenNotePicUrl(filteredData.description)
    }
  ...
  private async _noteFormatter(filteredData) {
    let content: string
    filteredData.commentUrl = await this.urlService.shortenUrl(filteredData.commentUrl).toPromise()
